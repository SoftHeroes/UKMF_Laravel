...
namespace App\Providers\accountManagment;
...
class SearchAccounts extends ServiceProvider
{
    /**
     * Register services.
     *
    ...
    public function searchAccount($request)
    {
        ...
        if (isEmpty($Language)) {
            ...
            return json_decode("{\"result\" :" . json_encode($queryResult[0]) . ",\"accounts\":null}");
        }
        ...
        $queryString = $queryString . (isEmpty($MiddleName) ? " AND 1 " : ("OR middleName LIKE '%" . $MiddleName . "%' "));
        $queryString = $queryString . (isEmpty($LastName) ? " AND 1 " : ("OR lastName LIKE '%" . $LastName . "%' "));
        $queryString = $queryString . (isEmpty($EmailAddress) ? " AND 1 " : ("OR emailID LIKE '%" . $EmailAddress . "%' "));
        $queryString = $queryString . (isEmpty($PhoneNumber) ? " AND 1 " : ("OR phoneNumber LIKE '%" . $PhoneNumber . "%' )"));
        ...
        return json_decode("{\"result\" :" . json_encode($queryResult[0]) . ",\"accounts\":" . json_encode($queryAccounts) . "}");
    }
