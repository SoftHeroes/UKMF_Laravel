...
DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
      ...
      SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00000' AND `language` = p_Language;
      ROLLBACK;
    END;
  ...
  BEGIN
    SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00012' AND `language` = 'English';
    LEAVE proc_Call;
  END;
  ELSEIF NOT EXISTS (select 1 from languageLookup where language = p_Language) THEN
    BEGIN
      SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00009' AND `language` = 'English';
      LEAVE proc_Call;
    END;
  END IF;
  ...
  IF(  p_Username IS NULL OR TRIM(p_Username) = '' ) THEN
    BEGIN
    SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00010' AND `language` = p_Language;
    LEAVE proc_Call;
    END;
  ELSEIF ( p_Source IS NULL OR TRIM(p_Source) = '' ) THEN
    BEGIN
      SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00022' AND `language` = p_Language;
      LEAVE proc_Call;
    END;
  ELSEIF ( p_Password IS NULL OR TRIM(p_Password) = '' ) THEN
    BEGIN
      SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00011' AND `language` = p_Language;
      LEAVE proc_Call;
    END;
  ELSEIF NOT EXISTS(  SELECT 1 FROM lookUp WHERE name = p_Source AND category = 'source' AND languageID =  getLanguageID(p_Language)  ) THEN
    BEGIN
          SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber  FROM `MessageMaster`  WHERE `Code` = 'ERR00033' AND `language` = p_Language;
          LEAVE proc_Call;
    END;
  END IF;
  ...
  IF( UserPhoneNumber IS NULL OR TRIM(UserPhoneNumber) = '' ) THEN
    BEGIN
       SELECT `Code`,`ErrorFound`,`Message`,`version`,`language`,ErrorMessage,UserEmail,UserPhoneNumber FROM `MessageMaster` WHERE `Code` = 'ERR00008' AND `language` = p_Language;
      LEAVE proc_Call;
    END;
  END IF;
